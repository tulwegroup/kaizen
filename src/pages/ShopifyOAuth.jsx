...
import { ShoppingBag, CheckCircle, XCircle, ChevronLeft, ExternalLink, Copy, Zap, Globe, Package, RefreshCw, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #f6f8f3 0%, #e3f0dc 100%)" }}>
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        {/* OAuth setup details (collapsed) */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <h3 className="font-semibold text-slate-900 mb-1">OAuth Configuration</h3>
          <p className="text-xs text-slate-500 mb-4">Use these in your Shopify Partner Dashboard if you need to reconfigure the app.</p>
          <div className="space-y-3">
            {["App URL", "Allowed redirect URL"].map(label => (
              <div key={label}>
                <p className="text-xs text-slate-400 font-medium uppercase tracking-wide mb-1">{label}</p>
                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2">
                  <p className="text-sm text-slate-700 flex-1 break-all font-mono">{stableUrl}</p>
                  <button onClick={copyUrl} className="flex-shrink-0 text-slate-400 hover:text-slate-700">
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
            <p className="text-xs text-slate-500">Start a fresh OAuth flow</p>
            <a href={`${stableUrl}?action=start`}>
              <Button size="sm" className="gap-1.5" style={{ background: "#008060" }}>
                <Zap className="w-3.5 h-3.5" /> Connect Store
              </Button>
            </a>
          </div>
        </div>
