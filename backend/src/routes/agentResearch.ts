/**
 * Agent Research — converts base44/functions/agentResearch/entry.ts
 */
...
const PERIOD_MULTIPLIERS: Record<string, number> = {
  '24h': 1 / 30, '1week': 7 / 30, '1month': 1, '3month': 3, '6month': 6, '1year': 12,
};
const PERIOD_LABELS: Record<string, string> = {
  '24h': '24 Hours', '1week': '1 Week', '1month': '1 Month', '3month': '3 Months', '6month': '6 Months', '1year': '1 Year',
};

router.post('/agent-research', adminOnly, async (req: AuthRequest, res: Response) => {
  try {
    const { regions, niches, period = '1month' } = req.body;
    if (!regions?.length) return res.status(400).json({ error: 'regions array required' });

    const nichesStr = niches?.length > 0
      ? niches.join(', ')
    ...
    const [productResearch, influencerResearch] = await Promise.all([
      ...
      invokeLLM({
        ...
        response_json_schema: influencerSchema,
      }),
    ]);

    const products: any[] = (productResearch.products as any[]) || [];
    ...
    res.json({
      ...
      total_projected_net_profit: profitProjections.reduce((s: number, p: any) => s + p.net_profit, 0),
    });
  } catch (err: any) {
